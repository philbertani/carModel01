...
class App {
  ...
  async initSceneGraphAndRender() {
    ...
    this.loadInfoDiv = document.getElementById("loadInfo");
    this.loadInfoDiv.innerText = "loading FBX";

    console.log("1",Date(Date.now()).toString());
    this.car = await loadFBX("toyotaMR2.fbx", this.loadInfoDiv);

    console.log("2",Date(Date.now()).toString());
    console.log(this.car);
    scene.add(this.car);

    console.log("3",Date(Date.now()).toString());
    ...
    scene.traverse((x) => {
      if (x.isMesh) {
        ...
        x.material.envMap = this.envMap.cubeMap01;
        x.material.reflectivity = .5;

        if (x.name.toUpperCase().includes("WIND")) {
          console.log("found a window",x.name);
          if (x.name === "WindshieldFront") {
            x.material.transparent = true;
            x.material.opacity = .1;
            x.material.reflectivity = .2;
            x.material.refractionRatio = .6;
            //x.material.visible = false;
            x.castShadow = false;
            x.receiveShadow = false;
            //console.log('zzzzzzzzzzzzzzzzzzz',x.material);
          }
        }
      }
    });
